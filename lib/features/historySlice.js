...
export const historySlice = createSlice({
  ...
  reducers: {
    addBrowsedProduct: (state, actions) => {
      { state.browsedProducts.findIndex(product=> product.id===actions.payload.id) == -1 ? state.browsedProducts = [...state.browsedProducts, actions.payload] : false };
    ...
    removeBrowsedProduct: (state, actions) => {
      ...
      console.log("browsedProducts", state.browsedProducts)
    },
  },
});
